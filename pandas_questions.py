...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    """
    counts = referendum_and_areas[["code_reg", 'name_reg',
                                  'Registered', 'Abstentions',
                                   'Null', 'Choice A', 'Choice B']]
    counts = counts.groupby(["code_reg", "name_reg"]).sum()
    return counts.reset_index("name_reg")
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    rrr = referendum_result_by_regions.reset_index("code_reg")
    merge = pd.merge(file, rrr, left_on="code", right_on="code_reg")
