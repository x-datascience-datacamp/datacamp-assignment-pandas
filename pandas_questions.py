...
def load_data():
    ...
    regions = pd.read_csv(
        'data/regions.csv', header=0, sep=',', error_bad_lines=False)
...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    regions_and_departments['name_dep'] = regions_and_departments[
        'name_dep'].str.replace(
            '-', ' ').str.replace(
                'ô', 'o').str.replace(
                    'é', 'e').str.replace('è', 'e').str.upper()
    merged_referendum_and_areas = pd.merge(
        regions_and_departments,
        referendum.rename(
            columns={
                'Department code': 'code_dep',
                'Department name': 'name_dep'}), on='name_dep', how='right')
    dom_tom_com = merged_referendum_and_areas[merged_referendum_and_areas[
        'code_dep_y'].str.startswith('Z')].index
    merged_referendum_and_areas = merged_referendum_and_areas.drop(dom_tom_com)
