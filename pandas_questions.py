...
To do that right, you will load the data as pandas.DataFrame, merge the info
and aggregate them by regions and finally plot them on a map using `geopandas`.
"""
import pandas as pd
...
def merge_regions_and_departments(regions, departments):
    ...
    data = regions[reg].merge(departments[dep], right_on="region_code",
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    results['ratio'] = \
        results['Choice A'] /\
        (results[['Choice A', 'Choice B']]).sum(axis=1)
    gpd.GeoDataFrame.plot(results, column='ratio')
    return results
