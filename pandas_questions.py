...
def merge_regions_and_departments(regions, departments):
    ...
    p.rename(columns={'region_code': 'code_reg', 
                                 'name_y': 'name_reg', 
                                 'code_x': 'code_dep', 
                                 'name_x': 'name_dep'}
              ,inplace=True)
...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    referendum=referendum[referendum["Department code"].isin(referendum["Department code"].unique()[:96])]  
    referendum
    dep=[]
    ...
    p=pd.merge(referendum, regions_and_departments,left_on='Department code'
               ,right_on='code_dep')
...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    """
    p=referendum_and_areas[['code_reg','name_reg', 'Registered', 'Abstentions', 'Null', 'Choice A', 'Choice B']]
    p=p.groupby(['code_reg']).sum()
    p=p.reset_index('code_reg')
  
    
    p=pd.merge(referendum_and_areas[['code_reg', 'name_reg']],p, on='code_reg')
    p=p.drop_duplicates()
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    fig=pd.merge(referendum_result_by_regions,data,left_on="code_reg",right_on="code")
